...
class BudgetService:
    ...
    def get_all_budgets_with_stats(self, user_id: int) -> list:
        ...
        with UnitOfWork() as uow:
            all_budgets = uow.budget.get_all_budgets_by_user(user_id)
            recurring_spent_map = uow.budget.get_recurring_spent_map(user_id)
            one_time_spent_map = uow.budget.get_one_time_spent_map(user_id)
    ...
    def create_or_update_budget(self, user_id: int, data: dict):
        ...
        with UnitOfWork() as uow:
            ...
            category = uow.categories.get_cat_by_id_and_user(category_id, user_id)
            if not category:
                raise ResourceNotFound(f"Category {category_id} not found or access denied.")

            budget_exist = uow.budget.get_by_category_and_user(user_id, category_id)

            if budget_exist:
                result = uow.budget.update_budget(budget_exist, payload)
                is_created = False
                logger.info(f"Budget updated for user {user_id} in category {category_id}")
            else:
                if uow.budget.get_count_by_user(user_id) >= MAX_BUDGET_PER_USER:
                    raise BusinessLogicError(f'You have reached the limit of {MAX_BUDGET_PER_USER} budgets')

                payload['user_id'] = user_id
                result = uow.budget.create_budget(payload)
                is_created = True
    ...
    def delete_budget(self, user_id, budget_id) -> bool:
        ...
        with UnitOfWork() as uow:
            ...
            budget_to_delete = uow.budget.get_by_id_and_user(budget_id, user_id)
            ...
            uow.budget.delete_budget(budget_to_delete)
            uow.commit()
        try:
            self._clear_related_caches(user_id)
