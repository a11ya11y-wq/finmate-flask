...
test.describe('Dashboard View', () => {
    test('Dashboard should display correct initial state for new user', async ({ page }) => {
        ...
        await step('Verify Toolbar components (title, filters, action buttons)', async () => {
        ...
        await step('Verify Stats Cards render with zero values', async () => {
        ...
        await step('Verify Charts containers are visible', async () => {
        ...
        await step('Verify Transactions Table is empty and pagination is disabled', async () => {
    ...
    test.describe('Charts', () => {
        test('Expense by Category chart are render correctly', async ({ page, api }) => {
            ...
            await step('Precondition: Create expenses in different categories', async () => {
            ...
            await step('Navigate to dashboard and verify chart screenshot', async () => {
                await dashboardPage.goto();
            ...
            await step('Verify legend items and calculated percentages', async () => {
        ...
        test('Balance Dynamics chart is rendered correctly', async ({ page, api }) => {
            ...
            await step('Navigate to dashboard and verify balance dynamics chart screenshot', async () => {
                await dashboardPage.goto();
    ...
    test('User can filter transactions', async ({ page, api }) => {
        ...
        await step('Precondition: Create transactions for different time periods', async () => {
            await api.transactions.createTransaction({
            ...
            await api.transactions.createTransaction({
                ...
                transaction_type: 'income',
                created_at: getDaysAgo(31),
                amount: 200,
                category_id: await api.categories.getCategoryIdByName('Salary'),
            });
        });

        await step('Navigate to dashboard and verify default "Month" filter state', async () => {
            await dashboardPage.goto();
            await expect(dashboardPage.transactionsTable.rows).toHaveCount(3);
        });

        await step('Apply "Week" filter and verify table rows and stat cards', async () => {
        ...
        await step('Apply "Month" filter and verify table rows and stat cards', async () => {
        ...
        await step('Apply "All time" filter and verify table rows and stat cards', async () => {
