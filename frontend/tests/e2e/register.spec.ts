...
test.describe('Register Actions', () => {
    ...
    for (const data of duplicateData) {
        test('User cannot register with duplicate data: ' + data.desc, async ({ page, request }) => {
            ...
            const originalEmail = 'uniqueemail' + Date.now() + '@example.com';
            const originalUsername = 'uniqueusername' + Date.now();
            ...
            await registerPage.goto();
            await registerPage.register({
                username: data.fieldToDuplicate === 'email' ? `existinguser${Date.now()}` : originalUsername, // Condition ? Value_If_True : Value_If_False
                email: data.fieldToDuplicate === 'email' ? originalEmail : `existinguser${Date.now()}@example.com`,
                password: 'Password123!',
                confirmPassword: 'Password123!'
            });
