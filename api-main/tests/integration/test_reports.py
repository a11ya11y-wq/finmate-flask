...
class TestGenerateReport:
    ...
    def test_generate_report_no_transactions(self, client, auth_headers, test_redis):
        ...
        with allure.step("Assert: Verify Redis queue remains empty"):
            queue_len = test_redis.llen("pdf_task_queue")
            assert queue_len == 0, f"Task should not be enqueued. Current queue length: {queue_len}"
    ...
    def test_generate_report_unauthorized(self, client):
        with allure.step("Act: Send POST without auth headers"):
            response = client.post('api/v1/report/generate-pdf', json=CREATE_REPORT_PAYLOAD)
    ...
    def test_generate_report_existing_pending(self, client, auth_headers, test_redis, arrange_tx):
        with allure.step("Arrange: Start initial report generation"):
            response1 = client.post('api/v1/report/generate-pdf', json=CREATE_REPORT_PAYLOAD, headers=auth_headers)
            assert response1.status_code == 202

        with allure.step("Act: Attempt to start generation for the same period again"):
            response2 = client.post('api/v1/report/generate-pdf', json=CREATE_REPORT_PAYLOAD, headers=auth_headers)
    ...
    def test_generate_report_existing_processed(self, client, auth_headers, test_redis, arrange_tx, db_session):
        ...
        with allure.step("Act: Request report for the same period"):
            response = client.post('api/v1/report/generate-pdf', json=CREATE_REPORT_PAYLOAD, headers=auth_headers)
    ...
    def test_generate_report_existing_failed(self, client, auth_headers, test_redis, arrange_tx, db_session):
        with allure.step("Arrange: Insert failed report attempt into DB"):
            report = Reports(user_id=1, start_date="2024-01-01", end_date="2026-01-31", status=ReportStatus.FAILED)
            db_session.add(report)
            db_session.flush()
    ...
    def test_generate_report_existing_expired(self, client, auth_headers, test_redis, arrange_tx, db_session):
        with allure.step("Arrange: Insert expired report into DB"):
            expired_report = Reports(
                user_id=1, start_date="2024-01-01", end_date="2026-01-31",
                status=ReportStatus.PROCESSED, file_url="http://example.com/expired_report.pdf",
    ...
    def test_generate_report_user_not_found(self, client, test_redis, mocker):
        ...
        with allure.step("Act: Request report generation"):
            response = client.post('api/v1/report/generate-pdf', json=CREATE_REPORT_PAYLOAD, headers=headers)
...
class TestGetReportStatus:
    ...
    def test_get_report_status_success(self, client, auth_headers, db_session):
        with allure.step("Arrange: Insert processed report"):
            report = Reports(user_id=1, start_date="2024-01-01", end_date="2026-01-31", status=ReportStatus.PROCESSED, file_url="http://example.com/report.pdf")
            db_session.add(report)
            db_session.flush()

        with allure.step("Act: Send GET request to status endpoint"):
            response = client.get(f'api/v1/report/generate-pdf/{report.id}/status', headers=auth_headers)
    ...
    def test_get_report_status_not_found(self, client, auth_headers):
        with allure.step("Act: Request status for ID 999"):
            response = client.get('api/v1/report/generate-pdf/999/status', headers=auth_headers)
    ...
    def test_get_report_status_invalid_id(self, client, auth_headers):
        with allure.step("Act: Request status with string ID instead of integer"):
            response = client.get('api/v1/report/generate-pdf/invalid_id/status', headers=auth_headers)
    ...
    def test_get_report_status_expired(self, client, auth_headers, db_session):
        ...
        with allure.step("Act: Request status"):
            response = client.get(f'api/v1/report/generate-pdf/{expired_report.id}/status', headers=auth_headers)
    ...
    def test_get_report_status_failed(self, client, auth_headers, db_session):
        with allure.step("Arrange: Insert FAILED report"):
            failed_report = Reports(user_id=1, start_date="2024-01-01", end_date="2026-01-31", status=ReportStatus.FAILED)
            db_session.add(failed_report)
            db_session.flush()

        with allure.step("Act: Request status"):
            response = client.get(f'api/v1/report/generate-pdf/{failed_report.id}/status', headers=auth_headers)
    ...
    def test_get_report_status_pending_timeout(self, client, auth_headers, db_session):
        with allure.step("Arrange: Insert PENDING report older than 2 hours"):
            pending_report = Reports(
                user_id=1, start_date="2024-01-01", end_date="2026-01-31",
                status=ReportStatus.PENDING, created_at=datetime.now(timezone.utc) - timedelta(hours=2)
            )
            db_session.add(pending_report)
            db_session.flush()

        with allure.step("Act: Request status"):
            response = client.get(f'api/v1/report/generate-pdf/{pending_report.id}/status', headers=auth_headers)
    ...
    @allure.title("Return 202 Accepted if PENDING report is still processing in background")
    @allure.severity(allure.severity_level.NORMAL)
    def test_get_report_status_pending_no_result(self, client, auth_headers, db_session, test_redis):
        with allure.step("Arrange: Insert recent PENDING report"):
            pending_report = Reports(
                user_id=1, start_date="2024-01-01", end_date="2026-01-31",
                status=ReportStatus.PENDING, created_at=datetime.now(timezone.utc) - timedelta(minutes=30)
            )
            db_session.add(pending_report)
            db_session.flush()
    ...
    @allure.title("Update DB and return 200 OK if PENDING report finished successfully in Redis")
    @allure.severity(allure.severity_level.CRITICAL)
    def test_get_report_status_pending_with_result(self, client, auth_headers, db_session, test_redis):
        with allure.step("Arrange: Insert PENDING report and mock Redis success result"):
            pending_report = Reports(
                user_id=1, start_date="2024-01-01", end_date="2026-01-31",
                status=ReportStatus.PENDING, created_at=datetime.now(timezone.utc) - timedelta(minutes=30)
            )
...
class TestGetReportHistory:
    ...
    def test_get_report_history_success(self, client, auth_headers, db_session, test_redis):
        with allure.step("Arrange: Insert various reports into DB"):
            report1 = Reports(user_id=1, start_date=datetime(2024, 1, 1), end_date=datetime(2026, 1, 31), status=ReportStatus.PROCESSED, file_url="http://example.com/report1.pdf")
            report2 = Reports(user_id=1, start_date=datetime(2023, 1, 1), end_date=datetime(2023, 12, 31), status=ReportStatus.FAILED)
    ...
    def test_get_report_history_reports_with_expired_links(self, client, auth_headers, db_session, test_redis):
        with allure.step("Arrange: Insert expired and valid reports"):
            expired_report = Reports(
                user_id=1, start_date=datetime(2024, 1, 1), end_date=datetime(2026, 1, 31),
                status=ReportStatus.PROCESSED, file_url="http://example.com/expired_report.pdf",
                expire_at=datetime.now(timezone.utc) - timedelta(days=1)
            )
            valid_report = Reports(
                user_id=1, start_date=datetime(2024, 1, 1), end_date=datetime(2026, 1, 31),
                status=ReportStatus.PROCESSED, file_url="http://example.com/valid_report.pdf",
    ...
    def test_get_report_history_reports_with_various_statuses(self, client, auth_headers, db_session, test_redis):
        with allure.step("Arrange: Insert PENDING, FAILED, and PROCESSED reports"):
            pending_report = Reports(user_id=1, start_date=datetime(2024, 1, 1), end_date=datetime(2026, 1, 31), status=ReportStatus.PENDING)
            failed_report = Reports(user_id=1, start_date=datetime(2024, 1, 1), end_date=datetime(2026, 1, 31), status=ReportStatus.FAILED)
            processed_report = Reports(user_id=1, start_date=datetime(2024, 1, 1), end_date=datetime(2026, 1, 31), status=ReportStatus.PROCESSED, file_url="http://example.com/processed_report.pdf")
