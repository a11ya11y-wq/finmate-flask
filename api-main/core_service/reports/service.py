...
class ReportService:
    ...
    def generate_pdf_report(self, user_id: int, data: dict) -> tuple[dict, int]:
        ...
        existing_report = self.uow.reports.get_active_report_by_period(
            user_id, start_date, end_date
        )
        if existing_report:
            logger.info(
                f"Existing report found for user_id: {user_id} in the specified period. Returning existing report."
            )

            if existing_report.status == ReportStatus.PROCESSED:
                if (
                    existing_report.expire_at
                    and datetime.now(timezone.utc) > existing_report.expire_at
                ):
                    logger.info(
                        f"Report {existing_report.id} expired. Generating a new one."
                    )
                    self.uow.reports.update_report_status(
                        existing_report.id, ReportStatus.EXPIRED, None
                    )
                    report = self.uow.reports.create_report(
                        user_id, start_date, end_date
                    )
                else:
                    return {
                ...
                logger.info(
                    f"Previous report generation failed or expired for user_id: {user_id}. Generating a new report."
                )
        ...
        transactions = self.uow.transactions.get_tx_by_period(
            user_id, start_date, end_date
        )

        # Calculate closing balance for the report period
        historical_sum_until_end = (
            self.uow.transactions.get_transactions_sum_until_date(user_id, end_date)
        )
        closing_balance = historical_sum_until_end + user.initial_balance

        # Calculate opening balance for the report period
        historical_sum_until_start = (
            self.uow.transactions.get_transactions_sum_until_date(
        ...
        if not transactions:
            logger.info(
                f"No transactions found for user_id: {user_id} in the specified period."
            )
        ...
        try:
            ...
            logger.info(
                f"Report generation task enqueued for user_id: {user_id} with report_id: {report.id}"
            )
        except Exception as e:
            logger.error(
                f"Failed to enqueue report generation task for user_id: {user_id} with report_id: {report.id}. Error: {str(e)}"
            )
            self.uow.reports.update_report_status(report.id, ReportStatus.FAILED)
            self.uow.flush()
            return {
                "error": "Failed to start report generation process. Please try again later."
            }, 400
    ...
    def get_report_status(self, user_id: int, report_id: int) -> tuple[dict, int]:
        ...
        if not report or report.user_id != user_id:
            logger.warning(
                f"Report with id {report_id} not found for user_id: {user_id}"
            )
        ...
        if report.status == ReportStatus.PROCESSED:
            ...
            if datetime.now(timezone.utc) > timeout_limit:
                logger.error(
                    f"Report {report_id} stuck in PENDING for too long. Marking as FAILED."
                )
        ...
        if status == "success":
            file_key = result_data.get("fileKey")

            if not file_key:
                logger.error(
                    f"Report {report_id} marked as success but no fileKey provided in Redis result."
                )
                self.uow.reports.update_report_status(report_id, ReportStatus.FAILED)
                self.uow.flush()
                self._clear_related_caches(user_id)
            ...
            updated_report = self.uow.reports.update_report_status(
                report_id, ReportStatus.PROCESSED, file_key, expire_time
            )
            self.uow.flush()
            self._clear_related_caches(user_id)
            ...
            return {
                ...
                "fileUrl": updated_report.file_url,
            }, 200
            ...
            return {"id": report_id, "status": status}, 202
    ...
    def get_report_history(self, user_id: int) -> list[dict]:
        ...
        for report in reports:
            ...
            if (
                ...
                and report.expire_at
                and datetime.now(timezone.utc) > report.expire_at
            ):
                report.status = ReportStatus.EXPIRED
                report.file_key = None
                self.uow.reports.update_report_status(
                    report.id, ReportStatus.EXPIRED, None, report.expire_at
                )
            ...
            report_history.append(
                {
                    ...
                    "fileUrl": display_url,
                    "expireAt": (
                        report.expire_at.isoformat() if report.expire_at else None
                    ),
                }
            )
