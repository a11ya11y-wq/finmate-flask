...
class AuthService:
    ...
    def login_user(self, data: dict, remember_me: bool = False) -> tuple:
        ...
        user = self.uow.auth.find_user_by_email(validated_data.email)
        ...
        self.uow.auth.update_refresh_token(user_id, refresh_token)
    ...
    def create_user(self, data: dict) -> Users:
        ...
        if self.uow.auth.find_user_by_email(validated_data.email):
            logger.warning(f"Registration failed: Email {validated_data.email} already exists")
            raise ConflictError("Email already registered.")

        if self.uow.auth.find_user_by_name(validated_data.username):
        ...
        new_user = self.uow.auth.create_user_with_cat(user_data_payload)
        self.uow.flush()
