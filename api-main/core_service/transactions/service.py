...
class TransactionService:
    ...
    def create_transaction(self, data: dict, user_id: int) -> Transactions:
        ...
        cat_obj = self.uow.categories.get_cat_by_id_and_user(category_id, user_id)
    ...
    def delete_transaction(self, tx_id: int, user_id: int) -> bool:
        ...
        tx_to_delete = self.uow.transactions.get_by_id_and_user(user_id, tx_id)
    ...
    def update_transaction(self, tx_id: int, user_id: int, data: dict) -> Transactions:
        ...
        tx_to_update = self.uow.transactions.get_by_id_and_user(user_id, tx_id)
        ...
        if tx_to_update.mono_id:
            ...
            for field in forbidden_fields:
                if field in update_payload:
                    ...
                    if field == 'amount':
                        if Decimal(str(new_value)) != Decimal(str(old_value)):
                            is_changed = True 
        ...
        if 'category_id' in update_payload:
            ...
            cat_obj = self.uow.categories.get_cat_by_id_and_user(new_cat_id, user_id)
    ...
    def get_transaction(self, tx_id: int, user_id: int) -> Transactions:
        transaction = self.uow.transactions.get_by_id_and_user(user_id, tx_id)
